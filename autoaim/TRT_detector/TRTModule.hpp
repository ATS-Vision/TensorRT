...
struct ArmorObject {
    cv::Point2f apex[4]; // [pt0, pt1, pt2, pt3]
    float prob;
    int color; // 0: blue, 1: red, 2: gray
    int cls;   // 0: guard, 1-5: number, 6: base
    int area;
//    std::vector <cv::Point2f> pts;

    cv::Point2f avg_pt; // Average corner position
    bool operator==(const ArmorObject &) const = default;

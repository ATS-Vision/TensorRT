...
bool TRTModule::detect(cv::Mat &src, std::vector <ArmorObject> &objects) {
...
// post-process [nms]
    objects.clear();
    objects.reserve(TOPK_NUM);
    std::vector<uint8_t> removed(TOPK_NUM);
    for (int i = 0; i < TOPK_NUM; i++) {
        auto *box_buffer = output_buffer + i * 20;
        if (box_buffer[8] < inv_sigmoid(KEEP_THRES)) break;
        if (removed[i]) continue;
        ArmorObject armor_obj;

        // Create a temporary vector to store the coordinates
        std::vector<cv::Point2f> temp_pts(4);

        memcpy(temp_pts.data(), box_buffer, 8 * sizeof(float));
        for (size_t j = 0; j < temp_pts.size(); ++j) {
            temp_pts[j].x *= fx;
            temp_pts[j].y *= fy;

            // Copy coordinates from temp_pts to armor_obj.apex
            armor_obj.apex[j] = temp_pts[j];
        }

        armor_obj.prob = sigmoid(box_buffer[8]);
        armor_obj.color = argmax(box_buffer + 9, 4);
        armor_obj.cls = argmax(box_buffer + 13, 7);
        objects.push_back(armor_obj);
    ...
    // Check if any armor objects were detected
    if (objects.empty()) {
        return false;
    } else {
        // Process the armor objects to calculate average corner points and areas
        for (ArmorObject &armor_obj: objects) {
            float avg_x = 0.0f, avg_y = 0.0f;
            for (const cv::Point2f &pt: armor_obj.apex) {
                avg_x += pt.x;
                avg_y += pt.y;
            }
            avg_x /= 4;
            ...
            // Calculate the area of the armor object.
            float width = std::sqrt(std::pow(armor_obj.apex[1].x - armor_obj.apex[0].x, 2) +
                                    std::pow(armor_obj.apex[1].y - armor_obj.apex[0].y, 2));
            float height = std::sqrt(std::pow(armor_obj.apex[3].x - armor_obj.apex[0].x, 2) +
                                     std::pow(armor_obj.apex[3].y - armor_obj.apex[0].y, 2));
            float area = width * height;

            // Set the average point and area of the armor object
            armor_obj.avg_pt = cv::Point2f(avg_x, avg_y);
            armor_obj.area = area;
        }
        return true;
    }
